...
- $B$ and $C$: If the red dice has a larger number than the white, we can only get odd numbers below that number. For example, if the red dice is 4, the white dice can only be 1 or 3. That means $P(A \cup B) \neq P(A) \cdot P(B)$ so the events are not independent.
